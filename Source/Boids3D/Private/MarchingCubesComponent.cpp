...
FVector UMarchingCubesComponent::GetPositionFromIndices(int x, int y, int z) const
{
...
bool UMarchingCubesComponent::PointIsOnContinuousBoundary(int x, int y, int z) const
{
	return 
...
void UMarchingCubesComponent::ComputeTriangles(TArray<TArray<FVector3f>>& Triangles)
{
	double t1 = FPlatformTime::Seconds();
...
void UMarchingCubesComponent::MakeStreamSet(FRealtimeMeshStreamSet& StreamSet, TArray<TArray<FVector3f>>& Triangles)
{
	double t1 = FPlatformTime::Seconds();
...
void UMarchingCubesComponent::MakeMesh(FRealtimeMeshStreamSet& StreamSet)
{
	double t1 = FPlatformTime::Seconds();
...
void UMarchingCubesComponent::ComputeMarchingCubes()
{
	...
	MakeStreamSet(StreamSet, Triangles);
	MakeMesh(StreamSet);
}
...
{
	bIsComputing = true;
	this->MarchingCubesComponent = InMarchingCubesComponent;

	this->RMC = MarchingCubesComponent->RealtimeMesh;
	...
	ShowPointsBelowThreshold = MarchingCubesComponent->ShowPointsBelowThreshold;
...
bool FMarchingCubesThread::PointIsOnContinuousBoundary(int x, int y, int z) const
{
	return
		(x == 0 && ContinuousBounds.XBack) ||
		(y == 0 && ContinuousBounds.YLeft) ||
...
FVector FMarchingCubesThread::GetPositionFromIndices(int x, int y, int z) const
{
	return FVector(MinCornerWorldSpace) + FVector(x, y, z) * CubeSize;
}